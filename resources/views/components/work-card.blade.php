...
<article class="work-card reveal"
  ...
  {{-- Browser frame --}}
  <div style="position:relative;background:var(--surface-2);padding:22px 22px 0;height:288px;overflow:hidden;">
    <div class="mini-browser" style="background:var(--card);border:1px solid var(--line);border-radius:12px 12px 0 0;box-shadow:0 20px 40px -26px rgba(13,46,73,.55);overflow:hidden;height:100%;">
      <div style="height:34px;display:flex;align-items:center;gap:6px;padding:0 13px;background:var(--card-bar);border-bottom:1px solid var(--line);">
        ...
        <span style="margin-left:10px;flex:1;height:19px;border-radius:6px;background:var(--surface-2);font-size:10.5px;color:var(--muted);font-weight:600;display:flex;align-items:center;padding:0 10px;overflow:hidden;white-space:nowrap;">{{ parse_url($item['url'], PHP_URL_HOST) }}</span>
      </div>
      <img class="shot" src="{{ asset('images/nitrodev/' . $item['image']) }}" alt="{{ $item['title'] }}">
    </div>
  </div>
